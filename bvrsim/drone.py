...
class f16(model):
    def __init__(self, lat, lon, alt, mach, num, head, pitch=0.0, roll=0.0, fuel=5e3):
        ...
        # 高度->俯仰角->升降舵
        self.alt2pitch = pid(self.fdm.get_delta_t(), 0.3, 0.01, 0.08)
        self.pitch2ele = pid(self.fdm.get_delta_t(), 3.5, 0.02, 0.18)
        # 马赫->油门
        self.mach2thr = pid(self.fdm.get_delta_t(), 2, 0.12, 0.05)
        self.AlarmList = []
