...
class aim120c(model):
    def __init__(self, drone: model, info: EnemyInfo):
        ...
        super().ready(lat, lon, alt, drone.Mach_M, head, min(90, 5 + pitch), 0, 113)
        self.maxalt = self.Altitude  # 最大高度
    ...
    def guide(self):
        ...
        if not self.state and dis < self.radarR:
            self.state = 1
        if not self.state:
            hor = float(np.linalg.norm(pos[:2]))  # 水平距离
            # 距离较远,高度下降小(升力够)
            if 2.5 * self.radarR < hor and self.maxalt - 500 < self.Altitude:
                # 高抛弹道虚拟目标点
                pos[2] = (self.Altitude < 14e3) * (pos[2] - min(5e3, hor / 10))
                dis = float(np.linalg.norm(pos))
        # 更新最大高度
        self.maxalt = max(self.maxalt, self.Altitude)
        vel -= self.vNED  # 绝对速度转相对速度
        los = pos / dis  # 单位向量
        # 径向接近速度
        ...
        N = 4  # 比例基准值
        if not self.state:
            vn = vel - np.dot(vel, los) * los  # 法向速度
            # 0控脱靶量中制导
            acmd = N * min(1, 0.8 + 0.02 * tgo) * (vn / tgo + an / 2)
        else:
            comp = N / 2 * an * min(1, tgo / 2)  # 目标机动补偿,近端衰减
            # APN末制导
    ...
    def step(self):
        ...
        super().step2(np.arctan2(acmd[1], -acmd[2]), -acmd[2] / g)
